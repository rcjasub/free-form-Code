...
export default function App() {
  ...
  const lastSelection = useRef<{ content: string; el: HTMLTextAreaElement } | null>(null)
  const canvasRef = useRef<HTMLDivElement>(null)

  function handleCanvasClick(e: React.MouseEvent<HTMLDivElement>) {
    if (e.target !== canvasRef.current) return
    const id = nextId.current++
    setNodes(prev => [...prev, { id, x: e.clientX, y: e.clientY, content: '' }])
  }
  ...
  useEffect(() => {
    async function handleKeyDown(e: KeyboardEvent) {
      ...
      try {
        ...
        setOutputs(prev => [...prev, { id: nextId.current++, x, y, text: data.output, isError: !!data.error }])
      } catch {
        setOutputs(prev => [...prev, { id: nextId.current++, x, y, text: 'Failed to reach server', isError: true }])
      }
    }
    ...
    >
      ...
      {nodes.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <p className="text-gray-300 text-sm font-mono select-none">click anywhere to start typing</p>
        </div>
      )}

      {/* toolbar */}
      <div className="absolute top-4 left-4 z-10 pointer-events-none">
        <span className="text-xs font-semibold text-gray-300 tracking-widest uppercase">free-form</span>
      </div>

      <div className="absolute top-4 right-4 z-10 pointer-events-none">
        <span className="text-xs text-gray-300 font-mono">select code + ctrl+enter to run</span>
      </div>
