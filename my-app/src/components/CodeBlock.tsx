...
export default function CodeBlock({ id, x, y, onDrag }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* output */}
      {output && (
        <div className={`px-3 py-2 font-mono text-xs whitespace-pre-wrap border-t ${isError ? 'bg-red-50 text-red-600 border-red-200' : 'bg-gray-950 text-gray-300 border-gray-800'}`}>
          {output}
        </div>
      )}
    </div>
